...
// check for no category selection
$(document).on('change','.product__category-checkbox',function() {
    let checkboxSelectedCount = $('.product__category .product__category-item input[type=checkbox]:checked').length;
    if(checkboxSelectedCount === 0) {
        $(elements.productWrapper).find('.product__list-category').addClass('product__list-category--active');
    }
});
...
$('.nav-mobile__button').on('click', function (e) {
    ...
    if ($('body').hasClass('body-disabled')) {
        $('.intro, .promo-carousel, .product__list-category').on('click', function (e) {
...
const hideShowAddButton = (element, quantity) => {
    ...
    if(element.target.offsetParent !== null) {
        ...
        $.each(elementList, (i, val) => {
            ...
            else if (quantity !== 0 && val.className === 'btn-increment__add') {
                val.style.zIndex = 0;
            }
...
const searchMobile = (control) => {
    if ($('.horizontal-scroll').is(':visible')) {
        $('.intro, .promo-carousel').hide();
        if (control) {
            $('.intro, .promo-carousel').show();
